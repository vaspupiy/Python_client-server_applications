...
def read_requests(r_clients: list, all_clients: list, data_volume: int, names: dict, chats: dict) -> dict:
    """ Чтение запросов списка клиентов"""
    responses = {}  # Словарь ответов сервера вида {socket: request}

    for sock in r_clients:
        try:
            b_data = receive_message_from_client(sock, data_volume)
            msg = parse_message(b_data)
            responses[sock] = msg
            if ('action' in msg) and (msg['action'] == 'presence') and (sock not in names):
...
@log
def write_responses(requests: dict, w_clients: list, all_clients: list, names: dict, chats: dict) -> None:
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                pass
            requests = read_requests(r, clients, _len_message, names, chats)
            if requests:
                write_responses(requests, w, clients, names, chats)
